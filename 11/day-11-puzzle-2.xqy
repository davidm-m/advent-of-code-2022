...
declare variable $LOWEST-COMMON-MULTIPLE := if ($TEST) then 96577 else 9699690;
...
declare function local:do-round($monkeys, $index) {
  ...
  then $monkeys
    ...
    let $new-scores := $increased-scores mod $LOWEST-COMMON-MULTIPLE
...
let $final-monkeys := local:do-rounds($monkeys, 10000)
