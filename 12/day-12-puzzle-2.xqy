...
declare function local:can-travel($from as xs:string, $to as xs:string) as xs:boolean {
  (fn:string-to-codepoints($from) - fn:string-to-codepoints($to)) le 1
};
...
let $start-unvisited :=
  ...
  return
    for $column in 1 to fn:count($rows[$row]/square)
    return
      if ($row eq $goal-row and $column eq $goal-column)
      then fn:string($row) || "," || fn:string($column) || ":0"
      else fn:string($row) || "," || fn:string($column) || ":10000"

let $all-distances := local:dijkstra((), $start-unvisited, fn:string($goal-row) || "," || fn:string($goal-column), $adjacency-map)

let $all-a-distances :=
  for $row in 1 to fn:count($rows)
  return
    for $column in 1 to fn:count($rows[$row]/square)
    let $distance := xs:int(fn:tokenize($all-distances[fn:starts-with(., fn:string($row) || "," || fn:string($column) || ":")], ":")[2])
    where $rows[$row]/square[$column] eq "a"
    return $distance

return fn:min($all-a-distances)